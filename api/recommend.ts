  ...
  try {
    ...
    if (!apiKey) {
      ...
      return res.status(500).json({
        error: "Vercel 대시보드(Settings -> Environment Variables)에 GEMINI_API_KEY가 등록되지 않았습니다. API 키를 추가 후 Redeploy 해주세요.",
      });
    }
...
`;
    ...
    console.error("Vercel API error in /api/recommend:", error);
    res.setHeader("Content-Type", "application/json");
    return res.status(500).json({
